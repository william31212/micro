...
int main()
{
	...
	char str[2][8] = {"40626216", "40626251"};
	//while(tmp != 0xFF)

	while(1)
	{
		tmp = (GPIO_PTC_PADIN >> 2) & ((1<<8) - 1);
		...
		if(tmp & 0x1)
		{
			for(i = 0; i < 8; i++)
			{
				for(k = 0; k < 300; k++)
				{
					...
					for(j = 0; j <= i; j++)
					{
						GPIO_PTD_GPIO = index_7LED_NUM[str[0][tmp--] - '0'];
			...
			while(tmp & 0x1)
			{
				for(j = 0; j < 8; j++)
				{
					GPIO_PTD_GPIO = index_7LED_NUM[str[0][7-j] - '0'];
					GPIO_PTA_GPIO = index_7LED[j];
					delay1(500);
				}
			}
		...
		else if (tmp & 0x2)
		{
			for(i = 7; i >= 0; i--)
			{
				for(k = 0; k < 300; k++)
				{
					...
					for(j = 7; j >= i; j--)
					{
						GPIO_PTD_GPIO = index_7LED_NUM[str[1][tmp++] - '0'];
						GPIO_PTA_GPIO = index_7LED[j];
						delay1(500);
					}
				}
			...
			while(tmp & 0x2)
			{
				for(j = 0; j < 8; j++)
				{
					GPIO_PTD_GPIO = index_7LED_NUM[str[0][j] - '0'];
